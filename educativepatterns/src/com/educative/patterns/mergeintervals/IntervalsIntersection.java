package com.educative.patterns.mergeintervals;
...
class IntervalsIntersection {
	...
	public static Interval[] merge(Interval[] arr1, Interval[] arr2) {
		...
		while (i < arr1.length && j < arr2.length) {
			...
			boolean b1 = arr1[i].start >= arr2[j].start && arr1[i].start <= arr2[j].end;
			boolean b2 = arr2[j].start >= arr1[i].start && arr2[j].start <= arr1[i].end;
			if ((b1) || (b2)) {
