...
class LargestPairs {
	...
	public static List<int[]> findKLargestPairs(int[] nums1, int[] nums2, int k) {
		//excellent. see this heap takes a pair and stores it in decreasing order of their sum
		...
		for (int i = 0; i < nums1.length && i < k; i++) {
			for (int j = 0; j < nums2.length && j < k; j++) {
				if (minHeap.size() < k) {
					...
					int[] minSumValue = minHeap.peek();
					if (nums1[i] + nums2[j] < minSumValue[0] + minSumValue[1]) {
						break;
