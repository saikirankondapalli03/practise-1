package com.educative.patterns.findkthmax;
...
public class KthSmallestNumberBrute {
		...
		for (int i = 0; i < k; i++) {
			for (int j = 0; j < nums.length; j++) {
				if (nums[j] > previousSmallestNum && nums[j] < currentSmallestNum) {
					...
					// can repeat,
					// we will consider 'nums[j]' only if it has a different index than previous
					// smallest
					currentSmallestNum = nums[j];
					currentSmallestIndex = j;
			...
			// current smallest number becomes previous smallest number for the next
			// iteration
	...
	public static void main(String[] args) {
		int result = KthSmallestNumberBrute.findKthSmallestNumber(new int[] { 1, 5, 12, 2, 11, 5 }, 4);
		System.out.println("Kth smallest number is: " + result);

		// since there are two 5s in the input array, our 3rd and 4th smallest numbers
		// should be a '5'
		result = KthSmallestNumberBrute.findKthSmallestNumber(new int[] { 1, 5, 12, 2, 11, 5 }, 4);
		System.out.println("Kth smallest number is: " + result);

		result = KthSmallestNumberBrute.findKthSmallestNumber(new int[] { 5, 12, 11, -1, 12 }, 3);
		System.out.println("Kth smallest number is: " + result);
	}
}
